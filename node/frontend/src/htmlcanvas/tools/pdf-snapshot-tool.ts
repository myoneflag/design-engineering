...
export const BORDERLESS_INFO_BAR_SIZE_MM = 0;
export const BORDER_INFO_BAR_SIZE_MM = 90;
export let INFO_BAR_SIZE_MM = 90;
...
export default class PdfSnapshotTool implements ToolHandler {
  ...
  static getScreenPaperScale(context: DrawingContext): number {
    ...
    return Math.min((sw - 100) / paperSize.widthMM, (sh - 250) / paperSize.heightMM);
  }
  ...
  beforeDraw(context: DrawingContext) {
    INFO_BAR_SIZE_MM = context.doc.uiState.exportSettings.borderless 
    ? BORDERLESS_INFO_BAR_SIZE_MM : BORDER_INFO_BAR_SIZE_MM;
