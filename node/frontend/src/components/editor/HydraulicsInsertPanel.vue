    ...
    <b-col>
      <b-button-group>
        <b-dropdown
          ...
          v-if="systemLayout === 'water'"
        >
          <b-dropdown-item variant="outline-dark" class="hot-water-plant btn-sm" @click="toggleHotWaterPlant">
          ...
          <b-dropdown-item variant="outline-dark" class="hot-water-plant btn-sm" @click="toggleTank">
            {{ selectedSystem.name + " Tank" }}
          </b-dropdown-item>

          <b-dropdown-item variant="outline-dark" class="hot-water-plant btn-sm" @click="togglePump">
        ...
        >
          ...
          <b-dropdown-item
            ...
            @click="
              $emit('insert', {
                entityName: entityNames.LOAD_NODE,
...
import { StandardFlowSystemUids, SupportedPsdStandards, isDrainage, isGas } from "../../../../common/src/api/config";
...
export default class HydraulicsInsertPanel extends Vue {
  mounted() {
    ...
    this.document.drawing.metadata.calculationParams.dwellingMethod !== null &&
      Mousetrap.bind(dwellingNodePair, this.toggleDwellingNodePair);

    this.availableFixtureList.map((fixture) => {
      this.hotKeySetting[fixture.uid] &&
        Mousetrap.bind(this.hotKeySetting[fixture.uid], () => this.toggleFixture(fixture.uid));
    });

    this.$props.availableValves
      .filter((vProps: { catalogId: string; type: string; name: string }) => {
      ...
      .map((vProps: { catalogId: string; type: string; name: string }) => {
        this.hotKeySetting[vProps.catalogId] &&
          Mousetrap.bind(this.hotKeySetting[vProps.catalogId], () => this.toggleValve(vProps));
      });
  }

  get systemLayout() {
    if (isDrainage(this.selectedSystem.uid, this.document.drawing.metadata.flowSystems)) {
      return "drainage";
    }

    if (isGas(this.selectedSystem.uid, this.catalog.fluids, this.document.drawing.metadata.flowSystems)) {
      return "gas";
    }
