...
export default class HydraulicsLayer extends LayerImplementation {
        ...
        for (let i = 0; i < this.uidsInOrder.length; i++) {
            ...
            if (!exclude.has(v)) {
                if (!active || !this.isSelected(v)) {
                    this.objectStore.get(v)!.draw(context, { active, selected: false, calculationFilters });
                }
            }
            if (i % 100 === 99) {
    ...
    resetDocument(doc: DocumentState) {
        ...
        const thisIds = entities.filter((e) => e[0].type !== EntityType.BACKGROUND_IMAGE).map((e) => e[0].uid);
        ...
        entities.forEach(([entity, levelUid]) => {
            ...
            if (this.objectStore.has(entity.uid)) {
                ...
                DrawableObjectFactory.buildVisible(
                    this,
                    () =>
                        levelUid ? doc.drawing.levels[levelUid].entities[entity.uid] : doc.drawing.shared[entity.uid],
                    this.objectStore,
                    {
        ...
        this.uidsInOrder = thisIds.sort((a, b) => {
            return (
                this.entitySortOrder(this.objectStore.get(a)!.entity) -
