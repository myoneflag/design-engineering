...
export function fillDefaultLoadNodeFields(
	...
	if (typeof result.customNodeId !== "undefined") {
		...
		result.minPressureKPA = (result.minPressureKPA === null && (node.minPressure
			? Number(node.minPressure)
			: null)) ||
			result.minPressureKPA;
		result.maxPressureKPA = (result.maxPressureKPA === null && (node.maxPressure
			? Number(node.maxPressure)
			: null)) ||
		...
		if (!result.node.loadingUnits || !result.node.designFlowRateLS || !result.node.continuousFlowLS) {
			for (var i = 0; i < node.fixtures.length; i++) {
				const selectedMaterialManufacturer = doc.drawing.metadata.catalog.fixtures.find(
					(obj) => obj.uid === node.fixtures[i]
				);
				...
				if (result.node.loadingUnits === null && isLUStandard(psdStrategy) && result.systemUidOption) {
					let systemChk = null;
					if (!!catalog.fixtures[node.fixtures[i]].loadingUnits[psdStrategy][result.systemUidOption]) {
						...
						!!catalog.fixtures[node.fixtures[i]].loadingUnits[psdStrategy]["warm-water"]
					) {
				...
				if (result.node.designFlowRateLS === null && result.systemUidOption) {
					...
					if (!!catalog.fixtures[node.fixtures[i]].qLS[manufacturer][selectedOption][result.systemUidOption]) {
						systemChk = result.systemUidOption;
					} else if (result.systemUidOption === "hot-water" &&
						!!catalog.fixtures[node.fixtures[i]].qLS[manufacturer][selectedOption]["warm-water"]) {
				...
				enDischargeUnits +=
					parseCatalogNumberExact(
						catalog.fixtures[node.fixtures[i]].enDrainageSystem[doc.drawing.metadata.calculationParams.drainageSystem]
					) || 0;
			}
		}
