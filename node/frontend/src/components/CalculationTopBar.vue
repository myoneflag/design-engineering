<template>
  <b-row style="position:fixed; left:350px; top:80px; max-width: 400px;">
    <b-col>
      ...
      <template v-else-if="calculationsUpToDate()">
        <table role="button" @click="settings">
          <tbody>
            <tr>
              <td>
                <v-icon
                  v-if="calculationsSuccess()"
                  id="calculation-check-success"
                  name="check"
                  scale="2"
                  color="green"
                ></v-icon>
                <v-icon v-else id="calculation-check-error" name="exclamation" scale="2" color="red"></v-icon>
...
export default class CalculationTopBar extends Vue {
  ...
  get drainageMethodName() {
    const drainageMethodName = DRAINAGE_METHOD_CHOICES.find((d) => d.key === this.calculationParams.drainageMethod)
      ?.name;
