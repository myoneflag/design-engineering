...
export default function insertLoadNode(
  ...
  loadingUnits: number = 1,
  continuousFlowLS: number = 0,
  dwellings: number = 1,
  variant: NodeVariant
) {
  const newUid = uuid();
    ...
    new SnappingInsertTool(
      ...
      (wc: Coord, event) => {
        ...
        const interactive = context.offerInteraction(
          ...
          (o) => {
            return o[0].type === EntityType.FITTING && context.globalStore.getConnections(o[0].uid).length <= 1;
          }
        );
        ...
        if (!event.ctrlKey) {
          connectLoadNodeToSource(context, context.globalStore.get(newEntity.uid) as LoadNode);
        }
        // rebaseAll(context);
