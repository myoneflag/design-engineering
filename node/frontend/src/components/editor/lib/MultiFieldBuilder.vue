...
import { ChoiceField, FieldType, PropertyField } from "../../../../../common/src/api/document/entities/property-field";
...
import { fillFlowSourceDefaults } from "../../../../../common/src/api/document/entities/flow-source-entity";
import {
  fillFixtureFields,
  makeFixtureFields
        ...
        fields.forEach((f) => {
          if (!seen.has(f.multiFieldId!)) {
            ...
            if (f.type === FieldType.Choice) {
              ...
              op.params.choices = op.params.choices.filter((c) => f.params.choices.find((cc) => cc.key === c.key));
            }
          }
        });
      } else {
    ...
    switch (obj.entity.type) {
      ...
      case EntityType.PLANT:
        return fillPlantDefaults(
          ...
          this.defaultCatalog,
          this.document.entityDependencies.get(obj.entity.uid)
        );
      case EntityType.FLOW_SOURCE:
        return fillFlowSourceDefaults(this.document.drawing, obj.entity);
      case EntityType.GAS_APPLIANCE:
        return fillGasApplianceFields(obj.entity, this.document.entityDependencies.get(obj.entity.uid));
    }
      ...
      {},
      {
        get: (target, name, receiver) => {
          ...
          const {
            concreteValue,
            concreteIdentical,
            allDefaultOrCalculated,
            someDefaultOrCalculated,
            foundField
