    ...
    <div>
        ...
        <b-container style="margin-top: 50px">
            ...
            <b-row style="padding-top: 20px">
                            ...
                            <b-nav pills vertical>
                                <template v-for="section in content">
                                    <template v-if="(typeof section.content) === 'string'">
                                        <b-nav-item :href="'#' + section.id" style="font-weight: bold">{{ section.title }}</b-nav-item>
                                    </template>
                                    <template v-else>
                                        <b-nav-item :href="'#' + section.id" style="font-weight: bold">{{ section.title }}</b-nav-item>
                                        <b-nav pills vertical small>
                                            <b-nav-item  class="ml-3 my-1" v-for="subsection in section.content" :href="'#' + subsection.id">{{ subsection.title }}</b-nav-item>
                                        </b-nav>
                                    </template>
                                </template>
                            </b-nav>
                ...
                <b-col cols="8">
                    <div id="scrollspy-nested" style="position:relative; height: calc(100vh - 180px); overflow-y:auto; text-align: left">
                        <template v-for="section in content">
                            <template v-if="(typeof section.content) === 'string'">
                            ...
                            <template v-else>
                                <h3 :id="section.id" style="margin-bottom: 25px; margin-top: 50px">{{ section.title }}</h3>
        ...
        get content(): Section[] {
            return [
...

                {
                    ...
                    content: [
                        ...
                        },
                        {
                            ...
                            id: "as3500-dewllings",
                            content: "<p>Note that fixtures in the dwelling will be sized based on the formula in AS3500.1:2018. Any pipes downstream of a dwelling will be sized based on the selected loading unit method.",
                        },
                        {
                            title: "Barrie’s Book Dwellings – Topic No. 4.7",
                            id: "barries-book-dwellings",
                            content: "<p>Note that fixtures downstream of dwellings will be sized based on Topic 4.2 of Barrie’s Book<p>" +
                        ...
                        },
                        {
                            ...
                            id: "din1998-300",
                            content: "Note this only works between 0.2-500 l/sec full flow, anything below will use full flow and anything above will be prohibited.",
                        }
                    ]
                },

                {
                    title: "Velocity",
                    id: "velocity",
                    content: "<p>Once the PSD is determined, the pipe size is selected based on the below formula which determines the velocity at the pipe size. One of the ways the software will size pipes is by finding the smallest pipe size at the maximum selected velocity.<p>" +
                    ...
                    content: [
                        {
                            ...
                            content: "<p>The Darcy Weisbach formula is used to determine friction loss in the pipe. This is preferred to Hazen Williams due to it not being as accurate; for example, Hazen Williams does not consider viscosity (temperature of water).<p>" +
...

                        {
                            ...
                            content: "<p>The pressure loss through a fitting is determined based on the k value of the fitting and at what velocity the water passes through the fitting.<p>" +
...

                        {
                            ...
                            content: "The pressure loss through the TMV is determined based on a graph - the higher the flow rate, the higher the friction loss.",
                        },

                        {
                            title: "Pressure Loss Through RPZD",
                            id: "pressure-loss-through-rpzd",
                            content: "<p>The pressure loss through the RPZD is determined based on the size of the RPZD and the associated graph - the higher the flow rate, the higher the friction loss. <p>" +
...

                {
                    ...
                    content: "<p>To get tempered water at an outlet, it usually requires a mixture of hot and cold water. To determine the PSD in the hot water system which uses flow rates to determine the PSD, we need to know what % of the tap flow rates comes from the hot water otherwise we would be oversizing.<p>" +
                    ...
                    content: [
                        {
                            title: "Disclaimer",
                            id: "disclaimer",
                            content: "<p>The information that contributes to the sizing and selecting of all components in the system can be found in the catalog. It is imperative that the catalog is reviewed and only used if the user agrees with the information.<p>" +
...

                        {
                            ...
                            content: "<p>You must provide your own design certification in addition to using the software. Our software is designed to be versatile and flexible, and so is unable to provide design certification. Some examples why:" +
                                "<ul>" +
...

                        {
                            title: "Common traps",
                            id: "common-traps",
                            content: "<p>We allow drawing systems freely, but as a result, there are times where you should be more aware:" +
                                "<ul>" +
