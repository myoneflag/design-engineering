...
async function drawTitleBar(
    ...
    pdf.text("Scale " + scaleName + " @" + paperSize.name, mm2pt(titleBarLeft + PADDING_MM), mm2pt(titleBarBottom), {
        width: mm2pt(INFO_BAR_SIZE_MM - PADDING_MM * 2)
    });

    // attribute table
    titleBarBottom -= pt2mm(
        drawGeneralTable(
            pdf,
            context,
            mm2pt(4),
    ...
    pdf.text(levelString, mm2pt(titleBarLeft + PADDING_MM), mm2pt(titleBarBottom), {
        width: mm2pt(INFO_BAR_SIZE_MM - PADDING_MM * 2)
    });
...
export async function drawScale(doc: PDFKit.PDFDocument, paperSize: PaperSize, scaleName: string) {
    ...
    SVGtoPDF(
        doc,
        scaled,
        mm2pt(paperSize.widthMM - MARGIN_SIZE_MM - INFO_BAR_SIZE_MM + PADDING_MM),
        mm2pt(paperSize.heightMM - MARGIN_SIZE_MM - 15),
        { assumePt: true }
    );
}
...
async function drawCoverSheet(context: CanvasContext, doc: PDFKit.PDFDocument, options: ExportPdfOptions) {
    ...
    if (calculationParams.psdMethod) {
        ...
        doc.text(context.effectiveCatalog.psdStandards[calculationParams.psdMethod].name, {
            align: "right",
            width: pageletContentWidth
        });
    }
    if (calculationParams.dwellingMethod) {
        ...
        doc.y = oy;
        doc.font(DEFAULT_FONT_NAME);
        doc.text(context.effectiveCatalog.dwellingStandards[calculationParams.dwellingMethod].name, {
            align: "right",
            width: pageletContentWidth
        });
    }

    {
        ...
        doc.y = oy;
        doc.font(DEFAULT_FONT_NAME);
        doc.text(PIPE_SIZING_METHODS.find((m) => m.key === calculationParams.pipeSizingMethod)!.name, {
            align: "right",
            width: pageletContentWidth
        });
    }

    {
        ...
        doc.font(DEFAULT_FONT_NAME);
        doc.text(
            RING_MAIN_CALCULATION_METHODS.find((m) => m.key === calculationParams.ringMainCalculationMethod)!.name,
            { align: "right", width: pageletContentWidth }
        );
    }

    {
        ...
        doc.font(DEFAULT_FONT_NAME);
        doc.text(
            COMPONENT_PRESSURE_LOSS_METHODS.find((m) => m.key === calculationParams.componentPressureLossMethod)!.name,
            { align: "right", width: pageletContentWidth }
        );
    }
...
export async function exportPdf(context: CanvasContext, viewPort: ViewPort, options: ExportPdfOptions) {
    ...
    const doc: PDFKit.PDFDocument = new (window as any).PDFDocument({
        autoFirstPage: false,
        width: options.paperSize.widthMM,
        height: options.paperSize.heightMM
    });
...
export async function snapshotToSvg(
    context: CanvasContext,
    viewPort: ViewPort,
    ...
    if (options.floorPlans) {
        if (canvasContext.globalStore.entitiesInLevel.has(canvasContext.document.uiState.levelUid!)) {
            for (const euid of canvasContext.globalStore.entitiesInLevel.get(
                canvasContext.document.uiState.levelUid!
            )!) {
