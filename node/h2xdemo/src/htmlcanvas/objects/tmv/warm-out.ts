...
import {WarmOutEntity} from '@/store/document/entities/tmv/tmv-entity';
...
@ConnectableObject
export default class WarmOut extends InvisibleNode<WarmOutEntity> {
    static register(): void {
        DrawableObjectFactory.registerEntity(EntityType.WARM_OUT, WarmOut);
    }
    ...
    offerInteraction(interaction: Interaction): boolean {
        switch (interaction.type) {
            ...
            case InteractionType.CONTINUING_PIPE:
                ...
                return interaction.system.uid === StandardFlowSystemUids.WarmWater;
    ...
    system(doc: DocumentState) {
        const system = doc.drawing.flowSystems.find((s) => s.uid === StandardFlowSystemUids.WarmWater);
        if (system) {
            return system;
        } else {
