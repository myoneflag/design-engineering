      ...
      <b-row>
        <b-col>
          ...
          >
            ...
            <b-input-group v-else-if="field[2] === 'number'" :prepend="field[3] ? convertUnits(field[3]) : undefined">
              <b-form-input
            ...
            <b-input-group v-else-if="field[2] === 'select'" :prepend="field[3] ? convertUnits(field[3]) : undefined">
              <b-form-select
            ...
            >
              <b-dropdown-item @click="setReactiveData(field[0], true)">
                Yes
              </b-dropdown-item>
              <b-dropdown-item @click="setReactiveData(field[0], false)">
                No
              </b-dropdown-item>
            </b-dropdown>

            <b-table-simple v-else-if="field[2] === 'array-table'" small fixed>
              <b-thead>
                <b-tr>
              ...
              <b-tbody>
                <b-tr v-for="(rowVal, rowIndex) in getReactiveData(field[0])" :key="rowIndex">
                  <b-td v-for="(col, colIndex) in field[3]" :key="rowIndex + '-' + colIndex">
                    ...
                    <b-input-group v-else :prepend="col.units ? convertUnits(col.units) : undefined" size="sm">
                      <b-form-input
                        size="sm"
            ...
            <b-table-simple v-else-if="field[2] === 'optional-numeric-table'" small>
              <b-thead>
                <b-tr>
                  <b-th>{{ field[4] }}</b-th>
                  <b-th>{{ field[1] }}</b-th>
                    ...
                    <b-form-checkbox
                      @change="
                        setOptionalTableRow(
                          field[0],
                          row,
                          $event ? undefined : getOriginalData(`${field[0]}.${row}`) || 0
                        )
                      "
...
export default class SettingsFieldBuilder extends Vue {
  ...
  setReactiveData(prop: string, value: any) {
    setPropertyByString(this.$props.reactiveData, prop, value);

    if (prop === "fluid") {
      this.$props.onChangeFluid();
    }
  ...
  displayWithCorrectUnits(units: Units | null, property: string): number | null {
    let result;
    if (units) {
      const val = convertMeasurementSystem(this.document.drawing.metadata.units, units, this.getReactiveData(property));
      result = val[1];
    } else {
  ...
  convertUnits(units: Units): Units {
    if (units) {
      const val = convertMeasurementSystem(this.document.drawing.metadata.units, units, 1);
      return val[0];
    } else {
  ...
  correctSelectedValue(field: any[], currValue: number | string) {
    let newVal: number | string =
      field[4].find((option: SelectField) => option.value == currValue)?.value || field[4][0].value;
