			...
			engine.flowGraph.dfs(
				...
				(node) => {
					...
					if (no.entity.type === EntityType.DIRECTED_VALVE && no.entity.valve.type === ValveType.GAS_REGULATOR) {
						...
						const filled = fillDefaultLoadNodeFields(
							engine.doc,
							engine.globalStore,
							no.entity,
							engine.catalog,
...
function getBestInsideDiameterLarge(
	inputRateCFH: number,
	pipeLengthFT: number,
	upstreamPSI: number,
	...
	return (
		inputRateCFH ** 0.381 /
		(18.93 * ((((upstreamPSI + 14.7) ** 2 - (downstreamPSI + 14.7) ** 2) * Y) / (cr * pipeLengthFT)) ** 0.206)
	);
}

export function sizeGasPipeInside(
	inputRateMJH: number,
	pipeLengthM: number,
	startKPA: number,
	endKPA: number,
	...
	const inputRateCFH = type === GasType.NATURAL_GAS ? inputRateMJH * 0.94782 : inputRateMJH / 2.62;
	...
	if (calculation.psdUnits) {
		if (calculation.psdUnits.gasMJH) {
				...
				for (const c of calculation.psdProfile.keys()) {
					...
					if (e.entity.type === EntityType.LOAD_NODE) {
						if (e.entity.node.type === NodeType.DWELLING) {
							...
							dwellingMJH += calculation.psdProfile.get(c)!.gasMJHUnDiversified;
						}
					}
				}
			}
          ...
          // https://www.elgas.com.au/blog/389-lpg-conversions-kg-litres-mj-kwh-and-m3
			...
			const res = (4000 * LS) / (Math.PI * parseCatalogNumberExact(calculation.realInternalDiameterMM)! ** 2);
