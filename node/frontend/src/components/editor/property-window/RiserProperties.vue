        ...
        <b-input-group size="sm" :key="slot">
          ...
          <b-input-group-append>
            <b-dropdown :text="selectedLevel[slot].name" size="sm" variant="info" class="riserLevelDropdown">
              <b-dropdown-item
                v-for="level in levelsToOptions"
                :key="level.text"
...
export default class RiserProperties extends Vue {
  ...
  get levelsToOptions() {
    return Object.values(this.levels).map((level: Level) => ({
      value: level.uid,
      text: level.name
    }));
  }
  ...
  isDisabled(field: PropertyField) {
    ...
    return (
      getPropertyByString(this.reactiveData, field.property, true) == null && (field.isCalculated || field.hasDefault)
    );
  }
